...
try {
    ...
    $controllers = $service->getAllControllersWithStatus($stripId, $length);
    
    // Récupérer les infos du ruban
    $ledStrip = $service->getLedStripById((int)$stripId);

    // Transformer en format JSON avec enrichissement PrestaShop
    $result = [];
    foreach ($controllers as $item) {
        // Chercher le produit dans PrestaShop par référence
        $psData = getPrestashopProductData($item['controller']->reference);
        
        // Utiliser les données PS si trouvées, sinon fallback sur JSON
        $price = $psData ? (float)$psData['price'] : $item['controller']->price;
        $stock = $psData ? (int)$psData['quantity'] : null;
        
        // Pour l'image : si pas d'image PS OU si l'image n'a pas d'ID, utiliser celle du JSON
        $imageUrl = $item['controller']->imageUrl ?? null;
        if ($psData && !empty($psData['id_image'])) {
            $imageUrl = $psData['image_url'];
        }
        
        $result[] = [
            'id' => $item['controller']->id,
            'name' => $item['controller']->name,
            'reference' => $item['controller']->reference,
            'image_url' => $imageUrl,
            'type' => $item['controller']->type,
            'usage' => $item['controller']->usage,
            'control_modes' => $item['controller']->controlModes,
            'voltage_min' => $item['controller']->voltageMin,
            'voltage_max' => $item['controller']->voltageMax,
            'max_power' => $item['controller']->maxPower,
            'max_pixels' => $item['controller']->maxPixels,
            'supported_chips' => $item['controller']->supportedChips,
            'wiring' => $item['controller']->wiring ?? 'pre-cable',
            'info' => $item['controller']->info ?? null,
            'price' => $price,
            'stock' => $stock,
    ...
    error_log('controllers-status.php error: ' . $e->getMessage());
}